...
int fat16_seek( void* private, uint32_t offset, FILE_SEEK_MODE seek_mode ) {
    ...
    // make sure offset is within the file
    struct fat_directory_item* ritem = desc_item->item;
    if( offset >= ritem->filesize ) return -EIO;

    // do the seek
    switch( seek_mode ) {
        case SEEK_SET: desc->pos = offset; return 0;
        case SEEK_END: return -EUNIMP; // not implemented
        case SEEK_CUR: desc->pos+= offset; return 0;
